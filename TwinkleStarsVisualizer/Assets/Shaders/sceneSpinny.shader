    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 getSimpleShading(float3 nor, int materialID, float d) {
                ...
                albedo *= 1. / d;
                float c0 = sin(materialID * 0.5) *.5+.5;
                float c1 = cos(materialID * 0.3 + 0.67) *.5+.5;
                albedo = float3(c0, 0., c1);
            ...
            float3 shootRays(float2 uv)
            {
                ...
                EYEPOS = float3(0.0, 0., 3.0);//float3(100 * STIME, 10., 100 * CTIME);
