...
public class AudioSpectrum : MonoBehaviour
{
    ...
    // public bool hitBeat = false;
    public float highestBeat4 = 0.0f;
    #endregion
    ...
    void Update ()
    {
        ...
        for (var bi = 0; bi < levels.Length; bi++) {
            ...
            levels [bi] = bandMax;
            if (peakLevels[bi] - falldown < bandMax)
            {

                if (bandMax > 0.05f && bi == 2)
                {
                    //hitBeat = true;
                    // Debug.Log(bandMax);
                    highestBeat4 = bandMax;
                }
                peakLevels[bi] = bandMax;
            }
            else
            {
                peakLevels[bi] = peakLevels[bi] - falldown;
            }
